...
| :--- | :--- | :--- | :--- | :--- |
| **Domain, Grid, and Layers (40 Points)** | Model domain, grid spacing ($\Delta x=\Delta y=50\text{ m}$) and  ($\Delta x=10\text{ m}$) and ($\Delta y=50\text{ m}$) at the river, and single-layer unconfined aquifer are all implemented correctly. | Minor errors in domain size or grid spacing. Aquifer setup is mostly correct. | Significant errors in model domain or grid setup, or incorrect layer type used. | Model domain, grid, and layer setup are fundamentally incorrect or missing. |
